[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/0bYioJ3m)
## Introduction
...
5. In a ***second command prompt window***, use the MongoDB tool (```mongorestore```) to import the saved albums' database (```albums.bson```) into your MongoDB datastore:
   ...
   ***NOTE:*** Make sure to place the path ```mongorestore.exe``` in double quotes that are separate from the double quotes for the ```<PATH TO albums.bson>```.
   <br><br>
   For example, if your ```albums.bson``` is located in: ```C:\Users\Me\MSOE\MongoDB\albums.bson``` your command would be:

   ```text
   "C:\Program Files\MongoDB\Tools\100\bin\mongorestore.exe" --db=discography "C:\Users\Me\MSOE\MongoDB\albums.bson"
   ```
...
8. In your terminal project, install package dependencies:
